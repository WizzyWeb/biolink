...
/**
 * Wraps provided HTML content in a resilient, table-based email HTML document suitable for broad client compatibility.
 *
 * The returned template includes MIME-friendly meta tags, Outlook conditional settings, inline CSS, responsive rules,
 * a branded header using `data.appName`, and a footer that references `data.supportEmail` and optionally `data.unsubscribeUrl`.
 *
 * @param content - HTML fragment to insert into the email body
 * @param data - Public template data used to populate dynamic sections (expects `appName`, `appUrl`, `supportEmail`, and optional `unsubscribeUrl`)
 * @returns A complete HTML email document string containing the provided content wrapped in the base email layout
 */
...
/**
 * Builds an account verification email template for a newly created user.
 *
 * @param data - Template data. Required fields: `appName`, `appUrl`, `supportEmail`, `verificationUrl`. Optional fields: `firstName`, `unsubscribeUrl`. `appName`, `appUrl`, and `supportEmail` are used to populate branding and contact sections.
 * @returns An EmailTemplate containing:
 *  - `subject`: the verification email subject line
 *  - `html`: a complete HTML email body (table-based, inline styles) with a verification CTA and fallback URL
 *  - `text`: a plain-text fallback message mirroring the HTML content
 */
...
/**
 * Builds an email template for resetting a user's password.
 *
 * The generated template includes an HTML body (with a prominent Reset Password button and a fallback URL),
 * a plain-text body, and a subject line incorporating the application name. The reset link in the message
 * is presented as expiring in 1 hour.
 *
 * @param data - Template data. Required fields on `data` include:
 *   - `resetUrl`: the URL the user should visit to reset their password.
 *   - `firstName` (optional): recipient's first name to personalize the greeting.
 *   Other `EmailTemplateData` fields (e.g., `appName`, `appUrl`, `supportEmail`, `unsubscribeUrl`) are used
 *   to populate the subject and footer content.
 * @returns An `EmailTemplate` containing `subject`, `html`, and `text` for the password reset email.
 */
...
/**
 * Builds the welcome email sent after a user creates and verifies their account.
 *
 * @param data - Email template data including `appName`, `appUrl`, `supportEmail`, optional `firstName`, and required `profileUrl` and `username`
 * @returns An `EmailTemplate` containing `subject`, `html`, and `text` for the welcome email
 */
...
/**
 * Builds an email notifying a user that their password was changed.
 *
 * The generated template includes a personalized greeting (when `firstName` is provided),
 * the change time, a security alert with support contact, and a link to the dashboard.
 *
 * @param data - Email template data including app information; must include `changeTime` and may include `firstName`
 * @returns An EmailTemplate containing `subject`, `html`, and `text` versions of the password-changed notification
 */
...
/**
 * Builds an account-locked notification email using the provided application and user data.
 *
 * @param data - EmailTemplateData plus:
 *   - `firstName?` — optional recipient first name shown in the greeting
 *   - `unlockUrl` — URL the recipient should visit to unlock their account
 *   - `lockReason` — short explanation of why the account was locked
 *   (EmailTemplateData must include `appName`, `appUrl`, and `supportEmail`; `unsubscribeUrl` is optional)
 * @returns An EmailTemplate containing the subject, an HTML body with an unlock button and support contact, and a plain-text fallback version.
 */
...
/**
 * Builds an email template to confirm a user's new email address.
 *
 * @param data - Email template data containing public site info and confirmation details. Must include:
 *   - `newEmail`: the address being confirmed
 *   - `confirmationUrl`: the link the user should follow to confirm
 *   - `firstName` (optional): recipient's first name for personalization
 *   Also uses `appName`, `appUrl`, and `supportEmail` from `data`; `unsubscribeUrl` is used if present.
 * @returns An EmailTemplate object with `subject`, HTML `html`, and plain-text `text` suitable for sending the confirmation email.
 */
...
/**
 * Builds a monthly newsletter email with both HTML and plain-text variants.
 *
 * The returned template uses `data.appName` and the current month/year in the subject,
 * includes a personalized greeting when `data.firstName` is provided, embeds
 * `data.newsletterContent` as the main newsletter body (may contain HTML), and
 * conditionally renders a "Featured This Month" section when `data.featuredLinks`
 * is supplied.
 *
 * @param data - Newsletter input:
 *   - `firstName` (optional): recipient's given name used for a personalized greeting.
 *   - `newsletterContent`: primary newsletter content; may include HTML blocks.
 *   - `featuredLinks` (optional): array of featured items with `title`, `url`, and `description` used to build a featured links section.
 * @returns The email template object with `subject`, `html`, and `text` properties suitable for sending.
...
/**
 * Builds an account deletion confirmation email tailored to the provided app and user data.
 *
 * @param data - Email template data including required `deletionDate` (the date when the account will be permanently deleted) and optional `firstName`; also uses `appName`, `appUrl`, and `supportEmail` to populate the message and action links.
 * @returns An EmailTemplate object containing the email `subject`, an HTML `html` body, and a plain-text `text` body for notifying the user that their account is scheduled for deletion and how to cancel it or contact support.
 */
...
/**
 * Return default EmailTemplateData configured for the LinkBoard application.
 *
 * @param appUrl - Base URL of the application; used to populate `appUrl` and to build the `unsubscribeUrl`
 * @returns An EmailTemplateData object with:
 * - `appName`: "LinkBoard"
 * - `appUrl`: the provided `appUrl`
