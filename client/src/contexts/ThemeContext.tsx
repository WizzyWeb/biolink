...
/**
 * Provides theme state and controls to descendant components and ensures the active theme's
 * CSS variables are applied to the document.
 *
 * The provider will attempt to load a profile-specific theme when `profileId` is provided;
 * if no profile ID is given or loading fails, it falls back to an internal default theme.
 *
 * @param profileId - Optional profile identifier used to load a custom theme; when omitted the default theme is used
 * @returns The ThemeContext provider element that supplies theme state and mutators to its children
 */
...
/**
 * Accesses the theme context value provided by the ThemeProvider.
 *
 * @returns The context value (`ThemeContextType`) containing the current `theme`, `isLoading`, `error`, and theme mutator methods.
 *
 * @throws Will throw an Error if called outside of a `ThemeProvider`.
 */
